                ...
                try {
                    if (!result.Item) return fn(null, null);
                        ...
                        if (!sess.S) {
                          ...
                          // TODO: FIXME: ConsistentRead issue
                          var tubiTrack = this.req.cookies.tubiTrack;
                          console.log(tubiTrack, sess.passport);
                          if (tubiTrack) {
                            var id = JSON.parse(tubiTrack).id;
                            var user = (sess.passport || {}).user;
                            if (user != id) {
                              console.log('Found session/cookie inconsistancy');
